<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="sass/a propos.css">
    <script type="text/javascript" src="js/propos.js"></script>
    <script src="https://kit.fontawesome.com/5e67adc73a.js" crossorigin="anonymous"></script>

    <title>a propos de moi</title>


</head>
<body>
<?php include 'navbar.php'; ?>
      <!-- ---------------a propos -------------------------- -->
    <div class="apropos">
        <img src="images/0P9A7577.jpg" alt="">
        <h1>À propos de moi</h1>
        <p>----------- Je suis développeur web freelance et j'aime ça ! -------------</p>
    </div>
    <div class="info">
        <div class="info_developer">
            <p>-je suis apprennent à l'école YOUCODE de développement web <br></br>

                -Je travaille actuellement en tant qu’intégratrice web, développeur 
                web et wordpress avec des clients  <br></br>
                
                -Passionnée depuis toujours par les nouvelles technologies 
                multimédia, je me suis également lancée en webdesign après une 
                formation en autodidacte. <br></br>
                
                -Que vous soyez une entreprise, une agence web ou un particulier,
                 je serai contente de répondre à vos besoins. <br></br>
                </p>
                <button>parler moi de vos projets</button>
        </div>
        <div class="info_image">
            <img src="images/developpeur_web_freelance_maroc_3.png" alt="">
        </div>
    </div>

    <!--Start Animated Progress Bar-->
    <div class="mescomce">
        <section class="mescomce_bar">
            <div class="compétences">
                <h2>Mes compétences   </h2>
            </div>
            <div class="mescomce_bar_articl">
                <div class="mescomce_bar_articl_nm">
                    <p>WEBDESIGN </p>
                </div>
                <div class="progress-bar">
                    <div class="label" data-count="90">100% </div>
                    <div class="line">
                      <span></span>
                    </div>
                </div>
            </div>
    
              
            <div class="mescomce_bar_articl">
                <div class="mescomce_bar_articl_nm">
                    <p>HTML & CSS</p>
                </div>
                <div class="progress-bar">
                    <div class="label" data-count="85">100% </div>
                    <div class="line">
                      <span></span>
                    </div>
                </div>
            </div>
              
            <div class="mescomce_bar_articl">
                <div class="mescomce_bar_articl_nm">
                    <p>BOOTSTRAP</p>
                </div>
                <div class="progress-bar">
                    <div class="label" data-count="80">100% </div>
                    <div class="line" >
                      <span></span>
                    </div>
                </div>
            </div>
              
            <div class="mescomce_bar_articl">
                <div class="mescomce_bar_articl_nm">
                    <p>JAVASCRIPT</p>
                </div>
                <div class="progress-bar">
                    <div class="label" data-count="70">100% </div>
                    <div class="line">
                      <span></span>
                    </div>
                </div>
            </div>

            <div class="mescomce_bar_articl">
                <div class="mescomce_bar_articl_nm">
                    <p>PHP/MYSQL</p>
                </div>
                <div class="progress-bar">
                    <div class="label" data-count="70">100% </div>
                    <div class="line" >
                      <span></span>
                    </div>
                </div>
            </div>

            <div class="mescomce_bar_articl">
                <div class="mescomce_bar_articl_nm">
                    <p>WORDPRESS</p>
                </div>
                <div class="progress-bar">
                    <div class="label" data-count="80">100% </div>
                    <div class="line">
                      <span></span>
                    </div>
                </div>
            </div>

            <div class="mescomce_bar_articl">
                <div class="mescomce_bar_articl_nm">
                    <p>GIT/GITHUB</p>
                </div>
                <div class="progress-bar">
                    <div class="label" data-count="80">100% </div>
                    <div class="line">
                      <span></span>
                    </div>
                </div>
            </div>
        </section>
    </div>

    <!--End Animated Progress Bar-->


    <!-- --mes servives-- -->

    <section class="messrv">
            <h2 class="messrv_h">MES SERVICES</h2>
        <div class="messrv_type">
            <div class="messrv_type_web" style="width: 25%;">
                <i class="fas fa-feather-alt"></i>
                <h2>WEBDESIGN</h2>
                <p>Créer des maquettes web, concevoir des interfaces (wireframes, prototypage), réfléchir à une identité graphique sur mesure, respecter une charte graphique, pour vous faire des identités visuelles uniques, qui vous représentent le mieux.
                </p>
            </div>
            <div class="messrv_type_front" style="width: 25%;">
                <i class="fas fa-code"></i>
                <h2>DÉVELOPPEMENT FRONT-END   </h2>
                <p>Intégrer des maquettes web, réfléchir sur la performance d’une page web, fournir du code propre pour une maquette peaufinée et un site utilisable sur un maximum d’appareils et navigateurs, c’est mon coeur de métier.
                </p>

            </div>
            <div class="messrv_type_word" style="width: 25%;">
                <i class="fab fa-wordpress-simple"></i>
                <h2>WORDPRESS</h2>
                <p>Personnaliser des templates, créer des thèmes avec des pages builders(Visual composer & Elemontor) ou realiser des thémes sur-mesure from scratch. WP est pour moi un outil pour mettre en place des sites faciles à mettre à jour et extensibles.
                </p>

            </div>
        </div>
    </section>

   




    
</body>
</html>